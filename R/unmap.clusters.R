...
function (results, path, mutation.data, position.data ){
  ...
  names(mutation.data.culled)<-mapply(function(x){substring(x,2)}, names(mutation.data.culled))
  for(i in 1:dim(results)[1]){
    ...
    start.sum <- which(names(mutation.data.culled) == results[i,2])
    end.sum <- which(names(mutation.data.culled ) == results[i,3])
